...
public class DeklaracijeIDefinicije_Sem {
	...
	public static void PROVJERI_definicija_funkcije(){
		...
		Tip_Const ime_tipa = Izrazi_Sem.PROVJERI_ime_tipa();
		...
		if (linija.equals("<lista_parametara>")){ 
			...
			NaredbenaStrukturaPrograma_Sem.PROVJERI_slozena_naredba();
			mSTZ.IzadjiIzCvora();
			return;
		}
	...
	public static void PROVJERI_init_deklarator(Tip_Const ime_tipa){
		...
		if (uz_op_pridruzi != null && uz_op_pridruzi.mNaziv.equals("OP_PRIDRUZI")){
			...
			if (Utilities_Sem.JeBrojevniTip(izravni_deklarator.mTip) && !izravni_deklarator.mNiz){ // nije niz
				if (!Utilities_Sem.ImplicitnaPretvorbaMoguca(inicijalizator.mLista.get(0).mTip, izravni_deklarator.mTip)) pogreska = true;
				if (inicijalizator.mJeNiz) pogreska = true;
			}else if (Utilities_Sem.JeBrojevniTip(izravni_deklarator.mTip) && izravni_deklarator.mNiz){ // je niz
				...
				for (int i = 0; i < inicijalizator.mLista.size(); ++i)
					if (!Utilities_Sem.ImplicitnaPretvorbaMoguca(inicijalizator.mLista.get(i).mTip, izravni_deklarator.mTip) ||
	...
	public static Tip_Const_Niz_BrEle_TipFunk PROVJERI_izravni_deklarator(Tip_Const ime_tipa){
		...
		if (uz_1.mNaziv.equals("L_UGL_ZAGRADA")){ // <<<< deklariranje nizova >>>>
			...
			if (linija.equals("<lista_parametara>")){ // -------- funkcija sa parametrima --------
				...
				if (clT != null){
					if (!Utilities_Sem.FunkcijeIste(clT.mTipFunkcija, noviCl.mTipFunkcija))
	...
	public static LISTA_Tip_LIzraz_Const_Niz PROVJERI_inicijalizator(){
		...
		if (linija.equals("<izraz_pridruzivanja>")){
			...
			if (Izrazi_Sem.mZadnjiPrimarniIzrazJe_NIZ_ZNAKOVA){
				for (int i = 0; i < Izrazi_Sem.mDuljina_NIZ_ZNAKOVA + 1; ++i){
